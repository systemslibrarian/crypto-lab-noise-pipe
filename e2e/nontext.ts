 ...
 *
 * In this repo the FIRST half is the live one. This is a control-dense page —
 * six tabs, 13 pattern chips, the guided-path pills, the anatomy token and
 * role buttons, the predict choices, two transport lanes of inputs and
 * buttons, six attack buttons, the compare chips and every clickable
 * comparison cell — and `main.css` styles most of them from the same few
 * tokens. The palette even documents the trap on `.btn-secondary`: its
 * `--bg-tertiary` fill "sits within ~1.1:1 of the surface behind it, so this
 * border is the only thing saying a control is here — the case SC 1.4.11
 * covers", which is why that rule was moved to `--control-border`. A comment
 * is not evidence, and nothing had ever measured whether the OTHER
 * `--bg-tertiary`-filled controls that still carry the decorative
 * `--border-color` edge (the guided-path pills, the anatomy token buttons,
 * the role switch) clear the same bar. This oracle measures the composite
 * each control is really drawn on, at every driven state, including `:hover`
 * — which persists after a click and is the state a reader occupies the
 * instant after pressing a Run-attack button.
 *
 * The SECOND half is inert here today, and that is a checked fact rather than
 * an assumption: `styles/main.css` contains no `content:` declaration — its
 * only `::before`/`::after` selector is the box-sizing reset. Every
 * non-colour cue on this page is a real character in the markup (the ✅/⚠/—/✗
 * badge glyphs, 🔒/🔓 on the wire blocks, the ⊘/→/⇄/◐/● security icons), so
 * `contrast.ts` walks them as ordinary text. The audit still runs on every
 * scan so the first `content:` rule someone adds is gated from the moment it
 * lands.
 *
 ...
 *    own background, or a border that actually lays paint down, or an outline.
 *    A plain text link carries its own identification and is not flagged, and
 *    neither is a control whose only "border" is the `1px solid transparent`
 *    that `.btn` and `.compare-cell` reserve so their hover state does not
 *    shift layout — a zero-alpha border paints nothing and cannot be an
 *    attempt at an edge, so it does not drag a text-identified control (the
 *    comparison table's property cells, the tab strip) into a judgement the
 *    SC does not make. It also cannot RESCUE a control that does paint a
 *    fill: the delineator test below composites the border over the fill, and
 *    a zero-alpha border just returns the fill.
 *
 ...
 *    the exact shape the `.btn-secondary` comment warns about, on a page
 *    where `.text-input` (fill `--bg-secondary`, within a whisker of the
 *    `--bg-card` lane behind it) depends on its `--control-border` edge the
 *    same way.
 *
 ...
 *    the wrong surface, and `escaped` is reported so triage can see it. With no
 *    `content:` rules in this lab the category is empty here today.
 *
 ...
 * background beneath — because the shared top bar's controls are bordered and
 * hover-filled with `color-mix()`, which `getComputedStyle` reports unresolved,
 * and reading the declared value would measure something the page never
 * paints.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * each gradient is *sampled* at the control's real location instead:
     * linear by projecting onto the gradient line, radial by distance from
     * the centre over the farthest-corner radius. A non-gradient layer
     * (`url()`, `none`) is unmeasurable and paints nothing.
     *
     * This lab paints no gradient at all — every surface a control sits on is
     * a flat custom-property colour — so both branches are inert here today.
     * The sampler is the fleet-shared paint model and is kept whole rather
     * than trimmed; see the matching note in `contrast.ts`.
     */
     ...
     *
     * The expensive part of a driven pass here is the generated-content half:
     * it asks for TWO extra computed styles (`::before` and `::after`) on
     * every element on the page, and a driven walkthrough step renders one
     * `.detail-row` (key span, hex span, copy button) per logged value while
     * the comparison table renders one `<button>` per property per selected
     * pattern. Without the caches the pass re-reads the same computed styles
    ...
    // Declared by the paint core shared with contrast.ts; not every helper is
    // reachable from both halves, and keeping the core identical between the
    // two files matters more than trimming the unused names.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
     ...
     *
     * The first version of this elsewhere in the fleet read `background-color`
     * alone, and that is not what every page paints: a lab whose canvas is a
     * `background` SHORTHAND of gradient layers reads `rgba(0, 0, 0, 0)` from
     * `backgroundColor`, and a colour-only walk then falls through to WHITE
     * and judges every control against a page that does not exist — the tell
     * was a disclosure-triangle ratio that came out IDENTICAL in both themes.
     * This lab happens to be flat everywhere (`html` carries a plain
     * `background-color: var(--bg-primary)`), so today the two readings agree
     * here; the sampler-backed form is kept because it is the reading that
     * stays correct if that ever changes, and because it borrows
     * `contrast.ts`'s model wholesale rather than approximating it a second
     * time.
     */
    ...
    const CONTROL = [
      ...
      // control?" test below. This lab's `.cross-badge` related-demo chips are
      // pill-shaped and filled, but they carry neither `btn` nor `button` in
      // their class, so they are judged as prose links; their fill is measured
      // all the same by the 1.4.3 walk in `contrast.ts`, which reads the ink
      // sitting on it.
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // A border only counts if it LAYS PAINT DOWN. `.btn` and `.compare-cell`
      // here ship `border: 1px solid transparent` purely so the hover state's
      // coloured border does not shift layout — a zero-alpha edge paints
      // nothing, is not an attempt at a boundary, and must not drag a
      // text-identified control into a judgement SC 1.4.11 does not make. The
      // hover state, where that border becomes `--accent-primary`, IS judged:
      // the drive scans with controls hovered and the alpha is real there.
      const borderInk = resolve(cs.borderTopColor) ?? TRANSPARENT;
      const hasBorder =
        bw.some((w) => w > 0) && cs.borderTopStyle !== 'none' && borderInk.a > 0;
